...
<template>
  <div :class="imageMap.classWrap"
    ...
    <div class="text-white absolute bottom-1 left-0 h-6 bg-black bg-opacity-65 rounded-lg z-20">
      <div class="flex items-center mx-2"><a href="https://unsplash.com/?utm_source=PicProse&amp;utm_medium=referral"
          target="_blank">
          Powered by
    ...
    <template v-else-if="coverInfo[item.key].length > 0">
      ...
      <template v-if="imageLayout === 'flex'">
        <ImageItem v-for="i in coverInfo[item.key]" :image="i" />
      </template>
    </template>
